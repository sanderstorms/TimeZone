...
namespace SqlTzLoader
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                if (_options.Verbose || args.Length == 1 || HelpRequired(args[0]))
                {
                    DisplayHelp();
                }
        ...
        private static void DisplayHelp()
        {
            ...
            Console.WriteLine("ProxyServer: {0}", _options.ProxyServer);
            Console.WriteLine("ProxyPort: {0}", _options.ProxyPort);
        }

        static async Task MainAsync(string[] args)
        {
            var ps = _options.ProxyServer;
            var pp = _options.ProxyPort;

            var tzdb = await CurrentTzdbProvider.LoadAsync(pp,ps);
